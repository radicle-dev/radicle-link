...
impl<'a, I: IdentityStorage> Evaluating<'a, I> {
        ...
        for (change, child_commits) in items {
            ...
            if let Some(reason) = self.evaluate_change(change) {
                ...
                match reason {
                    ...
                    RejectionReason::MissingAuthorizer { missing_auth_oid } => {
                        tracing::warn!(
                            commit=?change.commit(),
                            missing_authorizer_oid=?missing_auth_oid,
                            "rejecting change because the authorizer it referenced could not be found"
                        );
                    },
    ...
    fn evaluate_change(&mut self, change: &Change) -> Option<RejectionReason> {
        ...
        let referenced_auth_identity = match lookup_authorizing_identity(
            ...
            Ok(Some(id)) => id,
            Ok(None) => {
                return Some(RejectionReason::MissingAuthorizer {
                    missing_auth_oid: change.authorizing_identity_commit(),
                });
            },
            Err(e) => {
