...
async fn main() {
    ...
    task::spawn({
        ...
        async move { proto1.run(endpoint1, disco1).await }
    });
    task::spawn(async move { proto2.run(endpoint2, disco2).await });
    task::spawn(async move { proto3.run(endpoint3, disco3).await });
