    ...
    for peer in prune_list {
        match tracking::untrack(storage, urn, *peer, tracking::policy::Untrack::Any) {
            Ok(removed) => match removed {
                ...
                // We shouldn't hit this case using Untrack::Any
                Err(err) => {
                    tracing::trace!(err = %err, peer = %peer, "error encountered during previous value check");
                },
            },
            Err(err) => {
...
mod project {
    ...
    pub fn adopt_delegate_person(
        ...
        if storage.has_urn(&delegate_urn)? {
            ...
            track(storage, &delegate_urn, peer)?;
            track(storage, project_urn, peer)?;
        }
    ...
    fn track_direct(storage: &Storage, proj: &VerifiedProject) -> Result<(), Error> {
            ...
            .filter(|&key| key != local_peer_id.as_public_key())
        {
            ...
            match tracking::track(
                ...
                Ok(r) => {
                    tracing::trace!(peer = %peer, reference = %r.name, "tracked peer");
                },
                Err(err) => {
                    tracing::trace!(peer = %peer, err = %err, "tracking peer failed");
                },
            };
        }
...
pub fn track(storage: &Storage, urn: &Urn, peer: PeerId) -> Result<(), Error> {
    match tracking::track(
        storage,
        urn,
        Some(peer),
