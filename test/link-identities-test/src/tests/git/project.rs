...
use crate::helpers::{Device, Project};
...
fn double_vote() -> anyhow::Result<()> {
    let repo = tmp::repo()?;
    {
        ...
        // this at this level.
        let cheyenne_project_laptop = Project::create_from(cheyenne_laptop, &cheyenne_project)?;
        assert_matches!(
        ...
        assert_eq!(
            dylan_stupid.verify(lookup(&heads))?.into_inner().content_id,
            dylan_project.current().content_id
        );
...
fn fork() -> anyhow::Result<()> {
    let repo = tmp::repo()?;
    {
        ...
        let git = dylan.git::<identities::VerifiedProject>();
        anyhow::ensure!(
...
fn current_heads_from<'a>(
    devs: impl IntoIterator<Item = &'a Device<'a>>,
