...
the <<appendix_automerge>> for more about automerge. We first show how we map
the graph of automerge changes onto git in <<Change Commits>>, then we describe
how we reconstruct the change graph for an object given a repo in <<Reconstructing Collaborative Objects>>.
In <<Fetching Collaborative Objects>> we explain how the tips of collaborative
objects are stored under a particular reference and how those references are 
replicated to other peers. In <<Schema extensions>> we explain how we use JSON
schema to validate collaborative objects.
