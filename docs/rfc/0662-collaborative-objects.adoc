...
independently of the radicle protocol.
...
Furthermore a change commit MUST be signed by an identity which was at some point
a maintainer of the project (note that we can't stipulate that the identity is
_currently_ a maintainer as we do not know when the change was made). For
collaborative objects created in a direct identity (i.e a person identity) then
the change must be signed by the owner of the namespace.
...
`schema.json` contains a draft 2020-12 JSON schema whilst `manifest.toml` is a
TOML file with the following contents:
