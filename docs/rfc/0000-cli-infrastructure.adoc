...
binary on the users `$PATH` (or the Windows equivalent). This
...
* One binary is packages and distributed
...
The author recommends *2.*. The modularity provides a clean separation
of concerns and avoids a monolothic approach. We assume good faith
of overrides not happening [TODO: is there some way we can just
disallow this programatically?]. For packaging and distribution of the
binaries, we propose to have a script that will install each
subcommand binary — until we have a more well-founded way of
distributing Radicle artifacts.
...
We are noting here that -- at the time of writing -- we cannot publish
these binaries to <<crates.io>> due to the fact that `librad` and other
crucial libraries only live as a `git` repositories and are not on
crates.io. Anything that depends on them cannot be published as a
consequence.

This means that all subcommands will need to be built from source
initially, unless we build the binaries and distribute them from a
well-known source.
